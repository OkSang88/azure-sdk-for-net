// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System;

namespace SecurityInsights.Models
{
    /// <summary> Data type for Microsoft Threat Intelligence Platforms data connector. </summary>
    public partial class MstiDataConnectorDataTypesBingSafetyPhishingURL : DataConnectorDataTypeCommon
    {
        /// <summary> Initializes a new instance of MstiDataConnectorDataTypesBingSafetyPhishingURL. </summary>
        /// <param name="state"> Describe whether this data type connection is enabled or not. </param>
        /// <param name="lookbackPeriod"> lookback period. </param>
        /// <exception cref="ArgumentNullException"> <paramref name="lookbackPeriod"/> is null. </exception>
        public MstiDataConnectorDataTypesBingSafetyPhishingURL(DataTypeState state, string lookbackPeriod) : base(state)
        {
            if (lookbackPeriod == null)
            {
                throw new ArgumentNullException(nameof(lookbackPeriod));
            }

            LookbackPeriod = lookbackPeriod;
        }

        /// <summary> lookback period. </summary>
        public string LookbackPeriod { get; set; }
    }
}
