...
{
    public partial class ScheduledAlertRule : IUtf8JsonSerializable
    {
        ...
        internal static ScheduledAlertRule DeserializeScheduledAlertRule(JsonElement element)
        {
            ...
            return new ScheduledAlertRule(id, name, type, systemData, etag.Value, kind, Optional.ToNullable(queryFrequency), Optional.ToNullable(queryPeriod), Optional.ToNullable(triggerOperator), Optional.ToNullable(triggerThreshold), eventGroupingSettings.Value, alertRuleTemplateName.Value, templateVersion.Value, description.Value, query.Value, displayName.Value, Optional.ToNullable(enabled), Optional.ToNullable(lastModifiedUtc), Optional.ToNullable(suppressionDuration), Optional.ToNullable(suppressionEnabled), Optional.ToNullable(severity), Optional.ToList(tactics), incidentConfiguration.Value, Optional.ToDictionary(customDetails), Optional.ToList(entityMappings), alertDetailsOverride.Value);
        }
    }
}
