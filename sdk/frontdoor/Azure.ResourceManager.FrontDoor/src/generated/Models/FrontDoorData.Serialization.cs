...
namespace FrontDoorManagementClient
{
    public partial class FrontDoorData : IUtf8JsonSerializable
    {
        ...
        internal static FrontDoorData DeserializeFrontDoorData(JsonElement element)
        {
            ...
            return new FrontDoorData(id, name, type, tags, location, friendlyName.Value, Optional.ToList(routingRules), Optional.ToList(loadBalancingSettings), Optional.ToList(healthProbeSettings), Optional.ToList(backendPools), Optional.ToList(frontendEndpoints), backendPoolsSettings.Value, Optional.ToNullable(enabledState), Optional.ToNullable(resourceState), provisioningState.Value, cname.Value, frontdoorId.Value, Optional.ToList(rulesEngines));
        }
    }
}
