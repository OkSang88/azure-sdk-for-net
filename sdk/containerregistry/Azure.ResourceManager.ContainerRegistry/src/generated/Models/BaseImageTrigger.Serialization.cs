// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System.Text.Json;
using Azure.Core;

namespace ContainerRegistryManagementClient.Models
{
    public partial class BaseImageTrigger : IUtf8JsonSerializable
    {
        void IUtf8JsonSerializable.Write(Utf8JsonWriter writer)
        {
            writer.WriteStartObject();
            writer.WritePropertyName("baseImageTriggerType");
            writer.WriteStringValue(BaseImageTriggerType.ToString());
            if (Optional.IsDefined(UpdateTriggerEndpoint))
            {
                writer.WritePropertyName("updateTriggerEndpoint");
                writer.WriteStringValue(UpdateTriggerEndpoint);
            }
            if (Optional.IsDefined(UpdateTriggerPayloadType))
            {
                writer.WritePropertyName("updateTriggerPayloadType");
                writer.WriteStringValue(UpdateTriggerPayloadType.Value.ToString());
            }
            if (Optional.IsDefined(Status))
            {
                writer.WritePropertyName("status");
                writer.WriteStringValue(Status.Value.ToString());
            }
            writer.WritePropertyName("name");
            writer.WriteStringValue(Name);
            writer.WriteEndObject();
        }

        internal static BaseImageTrigger DeserializeBaseImageTrigger(JsonElement element)
        {
            BaseImageTriggerType baseImageTriggerType = default;
            Optional<string> updateTriggerEndpoint = default;
            Optional<UpdateTriggerPayloadType> updateTriggerPayloadType = default;
            Optional<TriggerStatus> status = default;
            string name = default;
            foreach (var property in element.EnumerateObject())
            {
                if (property.NameEquals("baseImageTriggerType"))
                {
                    baseImageTriggerType = new BaseImageTriggerType(property.Value.GetString());
                    continue;
                }
                if (property.NameEquals("updateTriggerEndpoint"))
                {
                    updateTriggerEndpoint = property.Value.GetString();
                    continue;
                }
                if (property.NameEquals("updateTriggerPayloadType"))
                {
                    if (property.Value.ValueKind == JsonValueKind.Null)
                    {
                        property.ThrowNonNullablePropertyIsNull();
                        continue;
                    }
                    updateTriggerPayloadType = new UpdateTriggerPayloadType(property.Value.GetString());
                    continue;
                }
                if (property.NameEquals("status"))
                {
                    if (property.Value.ValueKind == JsonValueKind.Null)
                    {
                        property.ThrowNonNullablePropertyIsNull();
                        continue;
                    }
                    status = new TriggerStatus(property.Value.GetString());
                    continue;
                }
                if (property.NameEquals("name"))
                {
                    name = property.Value.GetString();
                    continue;
                }
            }
            return new BaseImageTrigger(baseImageTriggerType, updateTriggerEndpoint.Value, Optional.ToNullable(updateTriggerPayloadType), Optional.ToNullable(status), name);
        }
    }
}
