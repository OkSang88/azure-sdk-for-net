// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System;
using System.Collections.Generic;
using Azure.Core;
using Azure.ResourceManager.Models;
using IotDpsClient.Models;

namespace IotDpsClient
{
    /// <summary> A class representing the ProvisioningServiceDescription data model. </summary>
    public partial class ProvisioningServiceDescriptionData : TrackedResource
    {
        /// <summary> Initializes a new instance of ProvisioningServiceDescriptionData. </summary>
        /// <param name="location"> The location. </param>
        /// <param name="properties"> Service specific properties for a provisioning service. </param>
        /// <param name="sku"> Sku info for a provisioning Service. </param>
        /// <exception cref="ArgumentNullException"> <paramref name="properties"/> or <paramref name="sku"/> is null. </exception>
        public ProvisioningServiceDescriptionData(AzureLocation location, IotDpsPropertiesDescription properties, IotDpsSkuInfo sku) : base(location)
        {
            if (properties == null)
            {
                throw new ArgumentNullException(nameof(properties));
            }
            if (sku == null)
            {
                throw new ArgumentNullException(nameof(sku));
            }

            Properties = properties;
            Sku = sku;
        }

        /// <summary> Initializes a new instance of ProvisioningServiceDescriptionData. </summary>
        /// <param name="id"> The id. </param>
        /// <param name="name"> The name. </param>
        /// <param name="type"> The type. </param>
        /// <param name="tags"> The tags. </param>
        /// <param name="location"> The location. </param>
        /// <param name="etag"> The Etag field is *not* required. If it is provided in the response body, it must also be provided as a header per the normal ETag convention. </param>
        /// <param name="properties"> Service specific properties for a provisioning service. </param>
        /// <param name="sku"> Sku info for a provisioning Service. </param>
        internal ProvisioningServiceDescriptionData(ResourceIdentifier id, string name, ResourceType type, IDictionary<string, string> tags, AzureLocation location, string etag, IotDpsPropertiesDescription properties, IotDpsSkuInfo sku) : base(id, name, type, tags, location)
        {
            Etag = etag;
            Properties = properties;
            Sku = sku;
        }

        /// <summary> The Etag field is *not* required. If it is provided in the response body, it must also be provided as a header per the normal ETag convention. </summary>
        public string Etag { get; set; }
        /// <summary> Service specific properties for a provisioning service. </summary>
        public IotDpsPropertiesDescription Properties { get; set; }
        /// <summary> Sku info for a provisioning Service. </summary>
        public IotDpsSkuInfo Sku { get; set; }
    }
}
