...
namespace PrivateDnsManagementClient
{
    public partial class RecordSetData : IUtf8JsonSerializable
    {
        ...
        internal static RecordSetData DeserializeRecordSetData(JsonElement element)
        {
            ...
            return new RecordSetData(id, name, type, etag.Value, Optional.ToDictionary(metadata), Optional.ToNullable(ttl), fqdn.Value, Optional.ToNullable(isAutoRegistered), Optional.ToList(aRecords), Optional.ToList(aaaaRecords), cnameRecord.Value, Optional.ToList(mxRecords), Optional.ToList(ptrRecords), soaRecord.Value, Optional.ToList(srvRecords), Optional.ToList(txtRecords));
        }
    }
}
