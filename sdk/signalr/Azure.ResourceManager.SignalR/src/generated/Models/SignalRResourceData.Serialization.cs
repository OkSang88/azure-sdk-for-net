...
namespace SignalRManagementClient
{
    public partial class SignalRResourceData : IUtf8JsonSerializable
    {
        ...
        internal static SignalRResourceData DeserializeSignalRResourceData(JsonElement element)
        {
            ...
            return new SignalRResourceData(id, name, type, tags, location, sku.Value, Optional.ToNullable(kind), identity.Value, systemData, Optional.ToNullable(provisioningState), externalIP.Value, hostName.Value, Optional.ToNullable(publicPort), Optional.ToNullable(serverPort), version.Value, Optional.ToList(privateEndpointConnections), Optional.ToList(sharedPrivateLinkResources), tls.Value, hostNamePrefix.Value, Optional.ToList(features), resourceLogConfiguration.Value, cors.Value, upstream.Value, networkACLs.Value, publicNetworkAccess.Value, Optional.ToNullable(disableLocalAuth), Optional.ToNullable(disableAadAuth));
        }
    }
}
