// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System;
using System.Net;
using System.Threading.Tasks;
using Azure.Core;
using Azure.Core.TestFramework;
using Azure.ResourceManager.Compute;
using Azure.ResourceManager.Compute.Models;
using Azure.ResourceManager.TestFramework;

namespace Azure.ResourceManager.Compute.Tests.Mock
{
    /// <summary> Test for ProximityPlacementGroup. </summary>
    public partial class ProximityPlacementGroupCollectionMockTests : MockTestBase
    {
        public ProximityPlacementGroupCollectionMockTests(bool isAsync) : base(isAsync, RecordedTestMode.Record)
        {
            ServicePointManager.ServerCertificateValidationCallback += (sender, cert, chain, sslPolicyErrors) => true;
            Environment.SetEnvironmentVariable("RESOURCE_MANAGER_URL", $"https://localhost:8443");
        }

        [RecordedTest]
        public async Task CreateOrUpdate()
        {
            // Example: Create or Update a proximity placement group.
            string proximityPlacementGroupName = "myProximityPlacementGroup";
            Compute.ProximityPlacementGroupData parameters = new Compute.ProximityPlacementGroupData(location: "westus")
            {
                ProximityPlacementGroupType = new Compute.Models.ProximityPlacementGroupType("Standard"),
            };

            var collection = GetArmClient().GetResourceGroup(new ResourceIdentifier("/subscriptions/00000000-0000-0000-0000-000000000000/resourceGroups/myResourceGroup")).GetProximityPlacementGroups();
            await collection.CreateOrUpdateAsync(proximityPlacementGroupName, parameters);
        }

        [RecordedTest]
        public async Task Get()
        {
            // Example: Create a proximity placement group.
            string proximityPlacementGroupName = "myProximityPlacementGroup";
            string includeColocationStatus = null;

            var collection = GetArmClient().GetResourceGroup(new ResourceIdentifier("/subscriptions/00000000-0000-0000-0000-000000000000/resourceGroups/myResourceGroup")).GetProximityPlacementGroups();
            await collection.GetAsync(proximityPlacementGroupName, includeColocationStatus);
        }

        [RecordedTest]
        public async Task GetAll()
        {
            // Example: Create a proximity placement group.

            var collection = GetArmClient().GetResourceGroup(new ResourceIdentifier("/subscriptions/00000000-0000-0000-0000-000000000000/resourceGroups/myResourceGroup")).GetProximityPlacementGroups();
            await foreach (var _ in collection.GetAllAsync())
            {
            }
        }
    }
}
