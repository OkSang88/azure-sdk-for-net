// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System.Text.Json;
using Azure.Core;

namespace AzureAnalysisServices.Models
{
    public partial class GatewayDetails : IUtf8JsonSerializable
    {
        void IUtf8JsonSerializable.Write(Utf8JsonWriter writer)
        {
            writer.WriteStartObject();
            if (Optional.IsDefined(GatewayResourceId))
            {
                writer.WritePropertyName("gatewayResourceId");
                writer.WriteStringValue(GatewayResourceId);
            }
            writer.WriteEndObject();
        }

        internal static GatewayDetails DeserializeGatewayDetails(JsonElement element)
        {
            Optional<string> gatewayResourceId = default;
            Optional<string> gatewayObjectId = default;
            Optional<string> dmtsClusterUri = default;
            foreach (var property in element.EnumerateObject())
            {
                if (property.NameEquals("gatewayResourceId"))
                {
                    gatewayResourceId = property.Value.GetString();
                    continue;
                }
                if (property.NameEquals("gatewayObjectId"))
                {
                    gatewayObjectId = property.Value.GetString();
                    continue;
                }
                if (property.NameEquals("dmtsClusterUri"))
                {
                    dmtsClusterUri = property.Value.GetString();
                    continue;
                }
            }
            return new GatewayDetails(gatewayResourceId.Value, gatewayObjectId.Value, dmtsClusterUri.Value);
        }
    }
}
