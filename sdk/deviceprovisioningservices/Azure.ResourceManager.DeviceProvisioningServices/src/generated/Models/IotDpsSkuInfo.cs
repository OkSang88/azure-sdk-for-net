// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

namespace IotDpsClient.Models
{
    /// <summary> List of possible provisioning service SKUs. </summary>
    public partial class IotDpsSkuInfo
    {
        /// <summary> Initializes a new instance of IotDpsSkuInfo. </summary>
        public IotDpsSkuInfo()
        {
        }

        /// <summary> Initializes a new instance of IotDpsSkuInfo. </summary>
        /// <param name="name"> Sku name. </param>
        /// <param name="tier"> Pricing tier name of the provisioning service. </param>
        /// <param name="capacity"> The number of units to provision. </param>
        internal IotDpsSkuInfo(IotDpsSku? name, string tier, long? capacity)
        {
            Name = name;
            Tier = tier;
            Capacity = capacity;
        }

        /// <summary> Sku name. </summary>
        public IotDpsSku? Name { get; set; }
        /// <summary> Pricing tier name of the provisioning service. </summary>
        public string Tier { get; }
        /// <summary> The number of units to provision. </summary>
        public long? Capacity { get; set; }
    }
}
