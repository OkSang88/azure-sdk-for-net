// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System.Text.Json;
using Azure.Core;

namespace DataFactoryManagementClient.Models
{
    public partial class AccessPolicyResponse
    {
        internal static AccessPolicyResponse DeserializeAccessPolicyResponse(JsonElement element)
        {
            Optional<UserAccessPolicy> policy = default;
            Optional<string> accessToken = default;
            Optional<string> dataPlaneUrl = default;
            foreach (var property in element.EnumerateObject())
            {
                if (property.NameEquals("policy"))
                {
                    if (property.Value.ValueKind == JsonValueKind.Null)
                    {
                        property.ThrowNonNullablePropertyIsNull();
                        continue;
                    }
                    policy = UserAccessPolicy.DeserializeUserAccessPolicy(property.Value);
                    continue;
                }
                if (property.NameEquals("accessToken"))
                {
                    accessToken = property.Value.GetString();
                    continue;
                }
                if (property.NameEquals("dataPlaneUrl"))
                {
                    dataPlaneUrl = property.Value.GetString();
                    continue;
                }
            }
            return new AccessPolicyResponse(policy.Value, accessToken.Value, dataPlaneUrl.Value);
        }
    }
}
