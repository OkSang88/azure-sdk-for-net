// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System;
using System.Net;
using System.Threading.Tasks;
using Azure.Core.TestFramework;
using Azure.ResourceManager.TestFramework;
using PostgreSQLManagementClient;

namespace PostgreSQLManagementClient.Tests.Mock
{
    /// <summary> Test for Database. </summary>
    public partial class DatabaseCollectionMockTests : MockTestBase
    {
        public DatabaseCollectionMockTests(bool isAsync) : base(isAsync, RecordedTestMode.Record)
        {
            ServicePointManager.ServerCertificateValidationCallback += (sender, cert, chain, sslPolicyErrors) => true;
            Environment.SetEnvironmentVariable("RESOURCE_MANAGER_URL", $"https://localhost:8443");
        }

        [RecordedTest]
        public async Task CreateOrUpdate()
        {
            // Example: Create a database
            string databaseName = "db1";
            PostgreSQLManagementClient.DatabaseData parameters = new PostgreSQLManagementClient.DatabaseData()
            {
                Charset = "utf8",
                Collation = "en_US.utf8",
            };

            var serverId = PostgreSQLManagementClient.Server.CreateResourceIdentifier("00000000-0000-0000-0000-000000000000", "TestGroup", "testserver");
            var collection = GetArmClient().GetServer(serverId).GetDatabases();
            await collection.CreateOrUpdateAsync(true, databaseName, parameters);
        }

        [RecordedTest]
        public async Task Get()
        {
            // Example: Get a database
            string databaseName = "db1";

            var serverId = PostgreSQLManagementClient.Server.CreateResourceIdentifier("00000000-0000-0000-0000-000000000000", "TestGroup", "testserver");
            var collection = GetArmClient().GetServer(serverId).GetDatabases();
            await collection.GetAsync(databaseName);
        }

        [RecordedTest]
        public async Task GetAll()
        {
            // Example: List databases in a server

            var serverId = PostgreSQLManagementClient.Server.CreateResourceIdentifier("00000000-0000-0000-0000-000000000000", "TestGroup", "testserver");
            var collection = GetArmClient().GetServer(serverId).GetDatabases();
            await foreach (var _ in collection.GetAllAsync())
            {
            }
        }
    }
}
