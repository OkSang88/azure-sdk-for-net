// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System.Text.Json;
using Azure.Core;

namespace LabServicesClient.Models
{
    public partial class LabServicesSkuCost
    {
        internal static LabServicesSkuCost DeserializeLabServicesSkuCost(JsonElement element)
        {
            Optional<string> meterId = default;
            Optional<float> quantity = default;
            Optional<string> extendedUnit = default;
            foreach (var property in element.EnumerateObject())
            {
                if (property.NameEquals("meterId"))
                {
                    meterId = property.Value.GetString();
                    continue;
                }
                if (property.NameEquals("quantity"))
                {
                    if (property.Value.ValueKind == JsonValueKind.Null)
                    {
                        property.ThrowNonNullablePropertyIsNull();
                        continue;
                    }
                    quantity = property.Value.GetSingle();
                    continue;
                }
                if (property.NameEquals("extendedUnit"))
                {
                    extendedUnit = property.Value.GetString();
                    continue;
                }
            }
            return new LabServicesSkuCost(meterId.Value, Optional.ToNullable(quantity), extendedUnit.Value);
        }
    }
}
