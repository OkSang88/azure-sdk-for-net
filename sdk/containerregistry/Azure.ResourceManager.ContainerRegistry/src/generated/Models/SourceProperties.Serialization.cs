// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System.Text.Json;
using Azure.Core;

namespace ContainerRegistryManagementClient.Models
{
    public partial class SourceProperties : IUtf8JsonSerializable
    {
        void IUtf8JsonSerializable.Write(Utf8JsonWriter writer)
        {
            writer.WriteStartObject();
            writer.WritePropertyName("sourceControlType");
            writer.WriteStringValue(SourceControlType.ToString());
            writer.WritePropertyName("repositoryUrl");
            writer.WriteStringValue(RepositoryUrl);
            if (Optional.IsDefined(Branch))
            {
                writer.WritePropertyName("branch");
                writer.WriteStringValue(Branch);
            }
            if (Optional.IsDefined(SourceControlAuthProperties))
            {
                writer.WritePropertyName("sourceControlAuthProperties");
                writer.WriteObjectValue(SourceControlAuthProperties);
            }
            writer.WriteEndObject();
        }

        internal static SourceProperties DeserializeSourceProperties(JsonElement element)
        {
            SourceControlType sourceControlType = default;
            string repositoryUrl = default;
            Optional<string> branch = default;
            Optional<AuthInfo> sourceControlAuthProperties = default;
            foreach (var property in element.EnumerateObject())
            {
                if (property.NameEquals("sourceControlType"))
                {
                    sourceControlType = new SourceControlType(property.Value.GetString());
                    continue;
                }
                if (property.NameEquals("repositoryUrl"))
                {
                    repositoryUrl = property.Value.GetString();
                    continue;
                }
                if (property.NameEquals("branch"))
                {
                    branch = property.Value.GetString();
                    continue;
                }
                if (property.NameEquals("sourceControlAuthProperties"))
                {
                    if (property.Value.ValueKind == JsonValueKind.Null)
                    {
                        property.ThrowNonNullablePropertyIsNull();
                        continue;
                    }
                    sourceControlAuthProperties = AuthInfo.DeserializeAuthInfo(property.Value);
                    continue;
                }
            }
            return new SourceProperties(sourceControlType, repositoryUrl, branch.Value, sourceControlAuthProperties.Value);
        }
    }
}
